...
const ROW_TONE = {
  flag: "bg-[#FDECEA] text-[#C0392B]",
  clear: "bg-[#F5F5F0] text-[#6B6B6B]",
};
...
function Stars({ score }) {
  ...
  return (
    <span className="flex items-center gap-1">
      <span className="flex text-[#FF6B2C]" aria-hidden="true">
        {[0, 1, 2, 3, 4].map((i) => {
      ...
      <span className="text-[11px] font-semibold tabular-nums text-[#6B6B6B]">{score}/5</span>
    </span>
  );
}
...
function Row({ row }) {
  ...
  return (
    <div className="rounded-lg border border-[#E8E8E4]">
      <button
        type="button"
        onClick={() => hasDetail && setOpen((v) => !v)}
        aria-expanded={hasDetail ? open : undefined}
        disabled={!hasDetail}
        className={`flex w-full items-center gap-2 px-2.5 py-2 text-left ${hasDetail ? "hover:bg-[#F5F5F0]" : "cursor-default"}`}
      >
        <span className="min-w-0 flex-1 truncate text-xs font-semibold text-[#1A1A1A]" title={row.hint || undefined}>
          {row.label}
        </span>
        ...
        {row.score != null ? (
          ...
          <span
            className={`shrink-0 rounded-md px-1.5 py-0.5 text-[11px] font-semibold whitespace-nowrap ${
              row.tone ? ROW_TONE[row.tone] || "text-[#6B6B6B]" : "text-[#6B6B6B]"
            }`}
          >
            {row.value}
          </span>
        ...
        {hasDetail && (
          <ChevronDown
            className={`h-3.5 w-3.5 shrink-0 text-[#9B9B9B] transition-transform ${open ? "rotate-180" : ""}`}
            aria-hidden="true"
          />
        )}
      </button>
      ...
      {open && (
        <ul className="space-y-1.5 border-t border-[#E8E8E4] px-2.5 py-2">
          {findings.map((f, i) => (
            <li key={i} className="flex gap-2 text-[11px] leading-relaxed">
              ...
              <span className={`min-w-0 ${TONE[f.tone] || TONE.neutral} ${f.tone === "note" ? "italic" : ""}`}>
                {f.text}
                {f.quote && (
                  <span className="mt-0.5 block border-l-2 border-[#E8E8E4] pl-2 text-[#6B6B6B] italic [overflow-wrap:anywhere]">
                    &ldquo;{f.quote}&rdquo;
                  </span>
                )}
              </span>
  ...
  return (
    <Card className="flex flex-col">
      <div className="mb-1 flex items-start justify-between gap-2">
        ...
        {badge && (
          <span
            className="shrink-0 rounded-md bg-[#FFE8DC] px-1.5 py-0.5 text-[11px] font-semibold text-brand-700"
            title={badge.detail}
          >
            {badge.label}
          </span>
      ...
      <p className="mb-3 text-[11px] text-[#6B6B6B]">Based on CV</p>
      <div className="space-y-2">
        {rows.map((r) => (
          <Row key={r.key} row={r} />
        ))}
      </div>
      {footer && <p className="mt-3 text-[11px] leading-relaxed text-[#6B6B6B]">{footer}</p>}
    </Card>
  );
}
