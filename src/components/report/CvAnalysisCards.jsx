...
const ROW_TONE = {
  flag: "bg-verdict-negative-tint text-verdict-negative",
  clear: "bg-canvas text-slate-600",
};
...
function Stars({ score }) {
  ...
  return (
    <span className="flex items-center gap-1">
      <span className="flex text-verdict-pending" aria-hidden="true">
        {[0, 1, 2, 3, 4].map((i) => {
      ...
      <span className="text-[11px] font-semibold tabular-nums text-slate-600">{score}/5</span>
    </span>
  );
}
  ...
  return (
    <div className="rounded-lg border border-slate-200">
      <button
        ...
        disabled={!hasDetail}
        className={`flex w-full items-center gap-2 px-2.5 py-2 text-left ${hasDetail ? "hover:bg-canvas" : "cursor-default"}`}
      >
        <span className="min-w-0 flex-1 truncate text-xs font-semibold text-slate-900" title={row.hint || undefined}>
          {row.label}
        </span>
        ...
        {row.score != null ? (
          ...
          <span
            className={`shrink-0 rounded-md px-1.5 py-0.5 text-[11px] font-semibold whitespace-nowrap ${
              row.tone ? ROW_TONE[row.tone] || "text-slate-600" : "text-slate-600"
            }`}
          >
            {row.value}
          </span>
          ...
          {findings.map((f, i) => (
            <li key={i} className="flex gap-2 text-[11px] leading-relaxed">
              ...
              <span className={`min-w-0 ${TONE[f.tone] || TONE.neutral} ${f.tone === "note" ? "italic" : ""}`}>
                {f.text}
                {f.quote && (
                  <span className="mt-0.5 block border-l-2 border-slate-200 pl-2 text-slate-500 italic [overflow-wrap:anywhere]">
                    &ldquo;{f.quote}&rdquo;
                  </span>
                )}
              </span>
  ...
  return (
    <Card className="flex flex-col">
      <div className="mb-1 flex items-start justify-between gap-2">
        ...
        {badge && (
          <span
            className="shrink-0 rounded-md bg-brand-50 px-1.5 py-0.5 text-[11px] font-semibold text-brand-700"
            title={badge.detail}
          >
            {badge.label}
          </span>
      ...
      <p className="mb-3 text-[11px] text-slate-500">Based on CV</p>
      <div className="space-y-2">
        {rows.map((r) => (
          <Row key={r.key} row={r} />
        ))}
      </div>
      {footer && <p className="mt-3 text-[11px] leading-relaxed text-slate-500">{footer}</p>}
    </Card>
  );
}
