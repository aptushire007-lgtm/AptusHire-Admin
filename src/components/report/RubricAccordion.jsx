...
function QuotePair({ probe }) {
  ...
  return (
    <div className="mt-3 grid gap-2 sm:grid-cols-2">
      {claim && (
        <figure className="min-w-0 rounded-lg border border-[#E8E8E4] bg-[#F5F5F0] p-2.5">
          <figcaption className="text-[10px] font-semibold tracking-[0.06em] text-[#9B9B9B] uppercase">
            What the CV claimed
          </figcaption>
          <blockquote className="mt-1 text-[11px] leading-relaxed text-[#6B6B6B] italic [overflow-wrap:anywhere]">
            &ldquo;{claim}&rdquo;
          </blockquote>
        </figure>
      )}
      {answer && (
        <figure className="min-w-0 rounded-lg border border-[#FFCAAF] bg-[#FFE8DC]/40 p-2.5">
...
function Row({ row, open, onToggle }) {
  ...
  return (
    <li className="border-b border-[#E8E8E4] last:border-b-0">
      <button
        type="button"
        onClick={onToggle}
        aria-expanded={open}
        className="tap-target flex w-full items-center gap-3 px-1 py-3 text-left transition-colors duration-150 hover:bg-[#FFE8DC]/50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-600"
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="flex flex-wrap items-center gap-x-2 gap-y-1">
            ...
            <span className="text-sm font-semibold text-[#1A1A1A] [overflow-wrap:anywhere]">{row.label}</span>
          </span>
          <span className="mt-0.5 block text-[11px] leading-snug text-[#6B6B6B]">{row.evidence}</span>
        </span>
        ...
        <ChevronDown
          className={`h-4 w-4 shrink-0 text-[#9B9B9B] transition-transform duration-200 ${open ? "rotate-180" : ""}`}
          aria-hidden="true"
        />
      </button>
        ...
        <div className="px-1 pb-4 pl-[1.1rem]">
          <dl className="flex flex-wrap gap-x-5 gap-y-1.5 text-[11px]">
            <div className="flex items-baseline gap-1.5">
              <dt className="text-[#6B6B6B]">Share of role</dt><dd className="font-bold tabular-nums text-[#1A1A1A]">{pctOf(row.weight)}%</dd>
            </div>
            {row.assessmentDetail?.itemCount > 0 && (
              <div className="flex items-baseline gap-1.5">
                <dt className="text-[#6B6B6B]">Test items</dt><dd className="font-semibold tabular-nums text-[#1A1A1A]">
          ...
          {row.untestedCause && UNTESTED_CAUSE_COPY[row.untestedCause] && (
            <p className="mt-3 rounded-lg bg-[#FFE8DC] px-3 py-2 text-[11px] leading-relaxed text-[#6B6B6B]">
...
export default function RubricAccordion({ coverage, id, provenance }) {
  ...
  return (
    <Card id={id} className="scroll-mt-32">
      <h3 className="font-display text-base font-bold tracking-tight text-[#1A1A1A]">
        Assessment Rubrics <span className="font-normal text-[#6B6B6B]">({all.length})</span>
      </h3>
      {provenance}

      <ul className="mt-3 border-t border-[#E8E8E4]">
        {rows.map((r) => (
