...
export default function TrendChart({ buckets, emptyLabel = "No applications yet" }) {
  ...
  if (!buckets.length || total === 0) {
    return (
      <div className="flex h-48 items-center justify-center rounded-card border border-dashed border-border bg-surface">
        <p className="text-xs font-medium text-text-faint">{emptyLabel}</p>
      </div>
    );
  }
  ...
  return (
    <figure className="mt-2">
      ...
      >
        {/* Subtle grid lines */}
        <g stroke="currentColor" className="text-border dark:text-slate-800/80" strokeWidth="1" vectorEffect="non-scaling-stroke">
      ...
      <figcaption className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs tabular-nums text-text-muted">
        <span>{buckets[0]?.label}</span>
        <span className="inline-flex items-center gap-1 rounded-full bg-lime-soft px-2.5 py-0.5 font-bold text-primary">
