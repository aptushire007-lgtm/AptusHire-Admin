  ...
  return (
    <Card>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-base font-semibold text-[#17221C]">Interview Rounds</h3>
      ...
      {inviting && (
        <form onSubmit={invite} className="mb-4 rounded-xl border border-[#E5EBE7] bg-[#E8F2EC]/60 p-4">
          ...
          <p className="mt-1 flex items-start gap-1.5 text-xs text-[#64736A]">
      ...
      {scorecards.length === 0 ? (
        ...
        <>
              ...
              {open.map((s) => (
                ...
                >
                  <div className="min-w-0 text-sm">
                    <span className="font-semibold text-[#17221C]">{s.interviewer.name}</span>
                    <span className="text-[#64736A]"> · {stageLabel(s.stage)}</span>
          ...
          {/* Submitted rounds */}
          {submitted.map((s) => (
            <div key={s.id} className="mb-3 rounded-xl border border-[#E5EBE7] p-3">
              ...
              {s.rollup?.withheldReason && (
                <p className="mt-1.5 text-xs text-[#64736A]">{s.rollup.withheldReason}</p>
              )}
              {s.decisionReason && (
              ...
              {s.rollup?.reproducibilityHash && (
                <p className="mt-2 flex items-center gap-1.5 text-[11px] text-[#64736A]">
          ...
          {ledger?.available && ledger.rounds.length > 0 && (
            <div className="mt-4">
              <h4 className="mb-2 text-sm font-semibold text-[#17221C]">Evidence ledger</h4>
              ...
              <div className="space-y-2.5">
                {ledger.criteria.map((c) => (
                  <div key={c.criterionId} className="rounded-xl border border-[#E5EBE7] p-3">
                    <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
                      <span className="text-sm font-semibold text-[#17221C]">{c.label}</span>
                      <span className="text-xs text-[#64736A]">{Math.round(c.weight * 100)}% of the rubric</span>
                    </div>
                    ...
                    <div className="mt-2.5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                      {c.cells.map((cell, i) => {
                        ...
                        return (
                          <div key={cell.roundId} className="rounded-lg bg-[#E8F2EC] px-3 py-2">
                            <p className="text-[11px] font-semibold text-[#64736A]">
                            ...
                            {!cell.assessed ? (
                              ...
                              <p className="mt-1 text-xs italic text-[#64736A]">not assessed</p>
                            ) : (
                              <>
                                <p className="mt-1 text-sm font-semibold text-[#17221C]">{cell.rating}/5</p>
                                {cell.note && <p className="mt-0.5 text-xs text-[#64736A]">{cell.note}</p>}
