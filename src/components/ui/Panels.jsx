...
export function PageHero({
  ...
  return (
    <Component
      className={`overflow-hidden rounded-3xl border border-slate-200/90 bg-gradient-to-r from-emerald-50/60 via-teal-50/30 to-white px-6 py-8 shadow-xs sm:px-8 sm:py-10 dark:border-slate-800/80 dark:bg-gradient-to-r dark:from-slate-900 dark:via-slate-900 dark:to-emerald-950/40 dark:shadow-soft ${className}`}
      {...props}
    >
      <div className="flex flex-wrap items-start justify-between gap-x-8 gap-y-5">
        <div className="min-w-0 max-w-2xl">
          {eyebrow && (
            <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-100/80 px-3 py-1 text-xs font-bold text-emerald-800 dark:border-emerald-800/60 dark:bg-emerald-950/80 dark:text-emerald-300">
              {EyebrowIcon && <EyebrowIcon className="h-3.5 w-3.5" aria-hidden="true" />}
              {eyebrow}
            </span>
          )}
          <h1
            className={`font-display text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white sm:text-3xl ${
              eyebrow ? "mt-3.5" : ""
            }`}
          >
            {title}
          </h1>
          {description && <p className="mt-2.5 max-w-prose text-sm leading-relaxed text-slate-600 dark:text-slate-300">{description}</p>}
        </div>
        {action && <div className="shrink-0">{action}</div>}
      </div>

      {points.length > 0 && (
        <ul className="mt-6 flex flex-wrap gap-x-6 gap-y-2.5 border-t border-slate-200/60 pt-5 dark:border-slate-800/60">
          {points.map((point) => (
            <li key={point} className="inline-flex items-center gap-2 text-xs font-semibold text-slate-700 dark:text-slate-300">
