...
function Pager({ page, total, limit, onPage }) {
  ...
  return (
    <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-border pt-3 text-sm text-text-muted">
      <span>
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          <p className="text-xs font-medium text-text-muted">LLM circuit breaker</p>
          <p className="mt-1"><Badge tone={breakerTone}>{data.breaker}</Badge></p>
        </Card>
        <Card>
          <p className="text-xs font-medium text-text-muted">Assessments (30d)</p>
          <p className="mt-1 text-2xl font-bold tracking-tight text-text-strong [overflow-wrap:anywhere]">{data.assessments.total}</p>
        </Card>
        <Card>
          <p className="text-xs font-medium text-text-muted">Review rate</p>
          <p className="mt-1 text-2xl font-bold tracking-tight text-text-strong [overflow-wrap:anywhere]">
            {data.assessments.reviewRate != null ? `${Math.round(data.assessments.reviewRate * 100)}%` : "—"}
          </p>
          <p className="text-[11px] text-text-muted">0% would mean overconfidence</p>
        </Card>
        <Card>
          <p className="text-xs font-medium text-text-muted">Ensemble agreement</p>
          <p className="mt-1 text-2xl font-bold tracking-tight text-text-strong [overflow-wrap:anywhere]">{data.assessments.meanEnsembleAgreement ?? "—"}</p>
        </Card>
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <h3 className="mb-3 text-base font-semibold text-text-strong">Counterfactual bias probes (30d)</h3>
        ...
        <Card>
          <h3 className="mb-3 text-base font-semibold text-text-strong">Scoring engine mix (30d)</h3>
          <div className="space-y-1.5 text-sm text-text-muted">
            {Object.entries(data.engineMix).length === 0 && <p className="text-text-muted">No scored candidates in range.</p>}
          ...
          <p className="mt-2 text-xs text-text-muted">fallback-legacy = the labelled no-AI path carried the decision — uncertainty stays visible.</p>
        </Card>
      </div>

      <Card>
        <h3 className="mb-3 text-base font-semibold text-text-strong">LLM spend by kind (30d)</h3>
        <RecordGrid>
          ...
          {data.spendByKind.length === 0 && <p className="text-sm text-text-muted">No LLM spend in range.</p>}
        </RecordGrid>
      </Card>

      <Card>
        <h3 className="mb-3 text-base font-semibold text-text-strong">Per-tenant assessment volume (30d)</h3>
        <div className="space-y-1.5 text-sm">
          {data.tenants.map((t) => (
            <div key={t.company} className="flex items-center justify-between gap-3 rounded-lg bg-canvas px-3 py-2">
              <span className="min-w-0 truncate text-text">{t.companyName}</span>
              <span className="shrink-0 text-xs text-text-muted">
                {t.assessments} assessed · review {t.reviewRate != null ? `${Math.round(t.reviewRate * 100)}%` : "—"}
              </span>
            </div>
          ))}
          {data.tenants.length === 0 && <p className="text-text-muted">No evidence-engine assessments in range.</p>}
        </div>
      </Card>
    </div>
  );
  ...
  return (
    <Card>
      ...
      {!data ? (
        ...
        <p className="py-8 text-center text-sm text-text-muted">No rows in range.</p>
      ) : (
        <>
          <RecordGrid columns={2}>
...
function DemoInterviewsTab() {
  ...
  return (
    <div className="space-y-6">
      <Card>
        <h3 className="mb-3 text-base font-semibold text-text-strong">Generate a demo interview link</h3>
        <FormGroup>
        ...
        {justCreated && (
          <div className="mt-4 rounded-xl border border-brand-200 bg-brand-50 p-4">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <code className="min-w-0 flex-1 truncate rounded-lg bg-surface px-3 py-2 text-xs text-text-muted">{justCreated.link}</code>
      ...
      <Card>
        <h3 className="mb-3 text-base font-semibold text-text-strong">Generated demo interviews</h3>
        {!data ? (
          <Skeleton className="h-40 w-full" />
        ) : data.items.length === 0 ? (
      ...
      <Modal open={Boolean(actionResult)} onClose={() => setActionResult(null)} size="md" title="New link generated" description="Copy it now — it won't be shown again.">
        <div className="flex flex-wrap items-center gap-2">
          <code className="min-w-0 flex-1 truncate rounded-lg bg-canvas px-3 py-2 text-xs text-text">{actionResult?.link}</code>
...
export default function PlatformConsole() {
  ...
  return (
    <div className="min-h-screen bg-canvas">
      <header className="sticky top-0 z-30 border-b border-border bg-surface px-5 py-3 shadow-card">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
          <p className="flex min-w-0 items-center gap-2 font-display text-base font-bold">
            <Cpu className="h-4.5 w-4.5 shrink-0 text-primary" aria-hidden="true" />
            <span className="truncate text-text-strong">AptusHire — Platform Console</span>
          </p>
          <button onClick={logout} className="flex shrink-0 items-center gap-1.5 text-sm text-text-muted hover:text-text-strong">
          ...
          {TABS.map((t) => (
            <button
              ...
              className={`flex items-center gap-1.5 rounded-xl border px-3.5 py-2 text-sm font-semibold transition ${
                tab === t.key ? "border-brand-500 bg-brand-50 text-brand-700" : "border-border bg-surface text-text-muted hover:bg-brand-50 hover:text-text"
              }`}
            >
