...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F0] text-[#1A1A1A] transition-colors">
      <MarketingNavbar />
      ...
      <section className="relative mx-auto grid max-w-7xl items-center gap-x-12 gap-y-16 px-5 pt-20 pb-28 sm:px-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,32rem)] lg:pt-24 lg:pb-36">
        <div className="max-w-2xl">
          <p className="flex items-center gap-2 text-sm font-semibold text-[#6B6B6B]">
          ...
          <h1 className="mt-5 text-4xl leading-[1.08] font-extrabold tracking-tight text-[#1A1A1A] [overflow-wrap:anywhere] sm:text-5xl lg:text-[3.4rem]">
            Every score traces
            <span className="block text-brand-600">to a quoted line.</span>
          </h1>
          <p className="mt-6 text-xl font-semibold text-[#1A1A1A] sm:text-2xl">
            The model never emits the score. Code computes it — from evidence it was required to cite.
          </p>
          <p className="mt-5 max-w-prose text-lg text-[#6B6B6B]">
      ...
      <section className="border-y border-[#E8E8E4] bg-[#F5F5F0]">
        <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:py-12">
          <p className="max-w-2xl text-lg font-semibold text-[#1A1A1A] [overflow-wrap:anywhere]">
            What you hand to the people who ask how a decision was made.
          </p>

          <div className="mt-8 grid gap-x-10 divide-y divide-[#E8E8E4] lg:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)_minmax(0,0.95fr)] lg:divide-x lg:divide-y-0">
            {EVIDENCE.map((item) => (
              <div key={item.title} className="py-5 first:pt-0 lg:py-0 lg:pl-10 lg:first:pl-0">
                <h2 className="font-semibold text-[#1A1A1A] [overflow-wrap:anywhere]">{item.title}</h2>
                <p className="mt-2 max-w-prose text-sm text-[#6B6B6B]">{item.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="how-it-works" className="mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:py-32">
        ...
        <div className="max-w-2xl">
          <h2 className="text-3xl font-bold tracking-tight text-[#1A1A1A] [overflow-wrap:anywhere] sm:text-4xl">
            ...
            return (
              <li key={step.stage} className="grid items-center gap-x-14 gap-y-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                <Reveal className={flip ? "lg:order-2" : undefined}>
                  ...
                  <h3 className="mt-2 text-2xl font-semibold tracking-tight text-[#1A1A1A] [overflow-wrap:anywhere]">
                    {step.title}
                  </h3>
                  <p className="mt-4 max-w-prose text-[#6B6B6B]">{step.body}</p>
                </Reveal>
      ...
      <section id="features" className="border-t border-[#E8E8E4] bg-[#F5F5F0]">
        <div className="mx-auto max-w-5xl px-5 py-24 sm:px-8">
          <h2 className="max-w-2xl text-3xl font-bold tracking-tight text-[#1A1A1A] [overflow-wrap:anywhere] sm:text-4xl">
            What the platform does
          </h2>
          <p className="mt-4 max-w-prose text-lg text-[#6B6B6B]">
            One workspace for jobs, candidates, interviews, and the record of how each decision was reached.
          </p>

          <dl className="mt-12 border-t border-[#E8E8E4]">
            {CAPABILITIES.map(([name, behaviour]) => (
              <div
                key={name}
                className="grid gap-x-10 gap-y-1.5 border-b border-[#E8E8E4] py-5 sm:grid-cols-[minmax(0,14rem)_minmax(0,1fr)]"
              >
                <dt className="font-semibold text-[#1A1A1A] [overflow-wrap:anywhere]">{name}</dt>
                <dd className="max-w-prose text-[#6B6B6B]">{behaviour}</dd>
              </div>
            ))}
          </dl>
        </div>
      ...
      <section id="why-us" className="mx-auto max-w-5xl px-5 py-24 sm:px-8">
        ...
        <div className="mt-12 grid gap-x-12 gap-y-10 sm:grid-cols-2">
          {REFUSALS.map(([title, body]) => (
            <div key={title}>
              <h3 className="text-lg font-semibold tracking-tight text-[#1A1A1A] [overflow-wrap:anywhere]">{title}</h3>
              <p className="mt-2 max-w-prose text-[#6B6B6B]">{body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="border-t border-[#E8E8E4] bg-[#F5F5F0]">
        <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
          <h2 className="max-w-2xl text-3xl font-bold tracking-tight text-[#1A1A1A] [overflow-wrap:anywhere] sm:text-4xl">
            Pricing
          </h2>
          <p className="mt-4 max-w-prose text-lg text-[#6B6B6B]">Start free. Move up as your hiring volume grows.</p>

          <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {plans.map((plan) => (
              <Tilt key={plan.key} className="flex flex-col rounded-2xl border border-[#E8E8E4] bg-white p-6 shadow-[0_1px_4px_rgba(27,67,50,0.07)]">
                <h3 className="font-semibold text-[#1A1A1A] [overflow-wrap:anywhere]">{plan.name}</h3>
                <p className="mt-1 text-sm text-[#6B6B6B]">{plan.description}</p>
                <p className="mt-5 text-2xl font-extrabold tabular-nums text-[#1A1A1A]">
                  {formatPrice(plan.pricing.monthly)}
                  {plan.pricing.monthly > 0 && <span className="text-sm font-medium text-[#6B6B6B]"> /mo</span>}
                </p>
                <ul className="mt-5 flex-1 space-y-2 border-t border-[#E8E8E4] pt-5 text-sm tabular-nums text-[#6B6B6B]">
            ...
            {plans.length === 0 && <p className="col-span-full text-sm text-[#6B6B6B]">Loading plans…</p>}
          </div>
        </div>
      </section>
      ...
      <footer id="contact" className="border-t border-[#E8E8E4] bg-white">
        <div className="mx-auto max-w-5xl px-5 py-16 sm:px-8">
          <p className="max-w-[38ch] text-2xl font-bold tracking-tight text-[#1A1A1A] [overflow-wrap:anywhere] sm:text-3xl">
            Hiring decisions are about people. They should be defensible in writing.
          </p>

          <div className="mt-12 flex flex-col justify-between gap-6 border-t border-[#E8E8E4] pt-6 text-sm sm:flex-row sm:items-end">
            <div>
              ...
              <p className="mt-3 text-[#6B6B6B]">© {new Date().getFullYear()} AptusHire. All rights reserved.</p>
            </div>

            <div className="flex flex-col gap-1.5 text-[#6B6B6B] sm:items-end">
              <a
